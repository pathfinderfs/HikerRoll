...
func TestGetEndedHikesByLeader(t *testing.T) {
	mux := setupTestMux()

	leader1UUID := "leader-ended-hikes-1"
	leader2UUID := "leader-ended-hikes-2"

	// Create some users (leaders)
	_, err := db.Exec(`INSERT OR IGNORE INTO users (uuid, name, phone) VALUES (?, ?, ?), (?, ?, ?)`,
		leader1UUID, "Leader One Ended", "111000111",
		leader2UUID, "Leader Two Ended", "222000222")
	require.NoError(t, err)

	// Create ended hikes for leader1
	hike1L1 := createTestHikeWithOptionsAndStartTime(t, User{UUID: leader1UUID}, "Ended Hike 1 L1", "Aiea Loop (upper)", time.Now().Add(-7*24*time.Hour))
	_, err = db.Exec("UPDATE hikes SET status = 'closed' WHERE join_code = ?", hike1L1.JoinCode)
	require.NoError(t, err)

	hike2L1 := createTestHikeWithOptionsAndStartTime(t, User{UUID: leader1UUID}, "Ended Hike 2 L1", "Diamond Head Crater (Le'ahi)", time.Now().Add(-5*24*time.Hour))
	_, err = db.Exec("UPDATE hikes SET status = 'closed' WHERE join_code = ?", hike2L1.JoinCode)
	require.NoError(t, err)

	// Create an open hike for leader1 (should not be returned)
	_ = createTestHikeWithOptionsAndStartTime(t, User{UUID: leader1UUID}, "Open Hike L1", "Koko Crater (Railway)", time.Now().Add(24*time.Hour))

	// Create an ended hike for leader2 (should not be returned for leader1)
	hike1L2 := createTestHikeWithOptionsAndStartTime(t, User{UUID: leader2UUID}, "Ended Hike 1 L2", "Friendship Garden", time.Now().Add(-3*24*time.Hour))
	_, err = db.Exec("UPDATE hikes SET status = 'closed' WHERE join_code = ?", hike1L2.JoinCode)
	require.NoError(t, err)

	// Test Case 1: Get ended hikes for leader1
	req1, _ := http.NewRequest("GET", fmt.Sprintf("/api/hikes/ended?leaderUUID=%s", leader1UUID), nil)
	rr1 := httptest.NewRecorder()
	mux.ServeHTTP(rr1, req1)

	assert.Equal(t, http.StatusOK, rr1.Code)
	var endedHikesL1 []Hike
	err = json.Unmarshal(rr1.Body.Bytes(), &endedHikesL1)
	require.NoError(t, err)
	assert.Len(t, endedHikesL1, 2, "Leader1 should have 2 ended hikes")

	foundHike1L1 := false
	foundHike2L1 := false
	for _, h := range endedHikesL1 {
		assert.Equal(t, "closed", h.Status, "All returned hikes should have status 'closed'")
		if h.JoinCode == hike1L1.JoinCode {
			foundHike1L1 = true
			assert.Equal(t, hike1L1.Name, h.Name)
		}
		if h.JoinCode == hike2L1.JoinCode {
			foundHike2L1 = true
			assert.Equal(t, hike2L1.Name, h.Name)
		}
	}
	assert.True(t, foundHike1L1, "Ended Hike 1 for L1 not found")
	assert.True(t, foundHike2L1, "Ended Hike 2 for L1 not found")

	// Test Case 2: Get ended hikes for leader2
	req2, _ := http.NewRequest("GET", fmt.Sprintf("/api/hikes/ended?leaderUUID=%s", leader2UUID), nil)
	rr2 := httptest.NewRecorder()
	mux.ServeHTTP(rr2, req2)

	assert.Equal(t, http.StatusOK, rr2.Code)
	var endedHikesL2 []Hike
	err = json.Unmarshal(rr2.Body.Bytes(), &endedHikesL2)
	require.NoError(t, err)
	assert.Len(t, endedHikesL2, 1, "Leader2 should have 1 ended hike")
	assert.Equal(t, hike1L2.JoinCode, endedHikesL2[0].JoinCode)
	assert.Equal(t, "closed", endedHikesL2[0].Status)

	// Test Case 3: Leader with no ended hikes
	leader3UUID := "leader-no-ended-hikes"
	_, err = db.Exec(`INSERT OR IGNORE INTO users (uuid, name, phone) VALUES (?, ?, ?)`, leader3UUID, "Leader Three No Ended", "333000333")
	require.NoError(t, err)
	_ = createTestHikeWithOptionsAndStartTime(t, User{UUID: leader3UUID}, "Open Hike L3", "Haha'ione", time.Now().Add(24*time.Hour)) // only open hike

	req3, _ := http.NewRequest("GET", fmt.Sprintf("/api/hikes/ended?leaderUUID=%s", leader3UUID), nil)
	rr3 := httptest.NewRecorder()
	mux.ServeHTTP(rr3, req3)

	assert.Equal(t, http.StatusOK, rr3.Code)
	var endedHikesL3 []Hike
	err = json.Unmarshal(rr3.Body.Bytes(), &endedHikesL3)
	require.NoError(t, err)
	assert.Len(t, endedHikesL3, 0, "Leader3 should have 0 ended hikes")

	// Test Case 4: Non-existent leaderUUID
	req4, _ := http.NewRequest("GET", "/api/hikes/ended?leaderUUID=nonexistent-leader-uuid", nil)
	rr4 := httptest.NewRecorder()
	mux.ServeHTTP(rr4, req4)

	assert.Equal(t, http.StatusOK, rr4.Code) // Endpoint returns 200 with empty list for unknown leader
	var endedHikesL4 []Hike
	err = json.Unmarshal(rr4.Body.Bytes(), &endedHikesL4)
	require.NoError(t, err)
	assert.Len(t, endedHikesL4, 0, "Non-existent leader should have 0 ended hikes")

	// Test Case 5: Missing leaderUUID parameter
	req5, _ := http.NewRequest("GET", "/api/hikes/ended", nil)
	rr5 := httptest.NewRecorder()
	mux.ServeHTTP(rr5, req5)

	assert.Equal(t, http.StatusBadRequest, rr5.Code, "Should return 400 Bad Request if leaderUUID is missing")
}
